...
var button$1 = {
  ...
  render: function render(h, _ref) {
    ...
    var staticClass = button + " " + button + "_" + props.type;
    var classList = {
      'is-mini': props.mini,
...
var Hairline = {
  ...
  render: function render(h, _ref) {
    ...
    var hairlines = placements.map(function (placement) {
      return h('div', {
        staticClass: hairline + "_line " + hairline + "_" + placement,
        style: hairlineStyle
      });
    });
...
var Item = {
  ...
  render: function render(h, _ref) {
    ...
    header && h('div', {
      staticClass: item + "_header"
    }, [header]), // Body
    h('div', {
      staticClass: item + "_body"
    }, [body || [// Left
    left && h('div', {
      staticClass: item + "_left"
    }, [left]), // Icon, Title, Desc, Extra
    h('div', {
      staticClass: item + "_middle"
    }, [title && h('div', {
      staticClass: item + "_info",
      style: "width: " + props.infoWidth + ";"
    }, [icon$$1 && h('div', {
      staticClass: item + "_icon"
    }, [_isString(icon$$1) ? h(Icon, {
      attrs: {
        name: icon$$1
      }
    }) : icon$$1]), title && h('div', {
      staticClass: item + "_outline"
    }, [h('div', {
      staticClass: item + "_title"
    }, [title]), desc && h('div', {
      staticClass: item + "_desc"
    }, [desc])])]), extra && h('div', {
      staticClass: item + "_extra"
    }, [extra])]), // Note, Right
    Boolean(note || right) && h('div', {
      staticClass: item + "_right"
    }, [note && h('div', {
      staticClass: item + "_note"
    }, [note]), right]), // Arrow
    h(Icon, {
      staticClass: item + "_arrow",
...
var List = {
  ...
  render: function render(h, _ref) {
    ...
    return h(Hairline, genFunctionalData(data, {
      ...
      staticClass: "" + list + (props.offset ? ' is-offset' : '')
    }), children);
  }
};
...
var inputNumber$1 = {
  ...
  computed: {
    ...
    Minus: function Minus() {
      var staticClass = inputNumber + "_button " + inputNumber + "_minus" + (this.minusDisabled ? ' is-disabled' : '');
    ...
    Plus: function Plus() {
      var staticClass = inputNumber + "_button " + inputNumber + "_plus" + (this.plusDisabled ? ' is-disabled' : '');
      return this.$createElement('a', {
        attrs: {
          href: 'javascript:;'
        },
    ...
    InputBox: function InputBox() {
      var staticClass = inputNumber + "_input-box" + (this.disabled ? ' is-disabled' : '');
      return this.$createElement('div', {
        staticClass: staticClass
      }, [this.$createElement(Input, {
        staticClass: inputNumber + "_input",
        attrs: {
          type: 'number',
...
var select$1 = {
  ...
  render: function render(h) {
    return h('select', {
      staticClass: select + " " + field + "--reset",
      domProps: {
